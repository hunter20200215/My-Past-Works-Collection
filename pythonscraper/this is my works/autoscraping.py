...
def add_csv_head():
    with open(output_file, 'w', newline='') as csv_file:
        ...
        writer.writerow(['Title', 'Email', 'PostID', 'Jobs_Category','Area', 'MapAddress', 'PostUrl', 'Compensation', 'EmploymentType', 'Phone'])

def add_csv_row(titel, emailaddress, PostID, jobs_category, area, mapaddress, posturl, compensation, employmenttype, Phone):
    # print("-----33--------")
    # print(jobs_category)
    with open(output_file, 'a', newline='', encoding="utf-8") as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow([title, emailaddress, PostID, jobs_category, area, mapaddress, posturl, compensation, employmenttype, Phone])
...
base_url = 'https://newyork.craigslist.org/search/que/jjj?s='

for i in range(25):
    url = base_url + str(i*120)
    print(url)
    ...
    for a in soup1.findAll('a', class_='result-title',  href=True):
        ...
        add_csv_row(title, emailaddress, PostID, jobs_category, area, mapaddress, posturl, compensation, employmenttype, Phone)
